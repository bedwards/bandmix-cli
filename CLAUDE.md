...
- CLI framework is **Click** with command groups.
- Data models use **Pydantic v2**.
- HTML parsing uses **BeautifulSoup4** with **lxml**.

## Dependency management

- Runtime deps go in `[project] dependencies`.
- Dev tools (pytest, ruff, pre-commit, responses) go in `[dependency-groups] dev`.
